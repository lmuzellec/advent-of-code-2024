...
std::vector<Equation> parseEquation(std::vector<std::string> input)
{
  ...
  while (it != input.end())
  {
    ...
    equation.e = std::stoull(prize[1].str());
    equation.f = std::stoull(prize[2].str());
                  ...
                  long double f)
{
  ...
  if (determinant != 0)
  {
    ...
    if (x > 0 && x < 100 && y > 0 && y < 100 && std::modf(x, &intpartx) == 0.0 && std::modf(y, &intparty) == 0.0)
    {
      return x * 3 + y;
    }
    return -1;
  ...
  {
    for (long double y = 99; y >= 0; y--)
    {
      long double x = (e - b * y) / a;
      if (x >= 0 && x < 100)
      {
        return x * 3 + y;
      }
