...
std::pair<std::unordered_map<Node, long long>, std::unordered_map<Node, std::set<Node>>> dijkstra(Graph &graph, Node start)
{
  std::unordered_map<Node, long long> distances;
  std::unordered_map<Node, std::set<Node>> previous;
  ...
  distances[start] = 0;
  previous[start] = std::set<Node>();

  std::priority_queue<CostToNode, std::vector<CostToNode>, std::greater<CostToNode>>
      priority_queue;
  ...
  while (!priority_queue.empty())
  {
    ...
    for (auto neighbor : graph.connections[currentNode])
    {
      ...
      if (newDistance < distances[neighborNode])
      {
        ...
        previous[neighborNode] = std::set<Node>({currentNode});
      ...
      if (newDistance == distances[neighborNode])
      {
        previous[neighborNode].insert(currentNode);
      }
    }
  }
...
long long solve(std::vector<std::string> input)
{
  ...
  std::set<std::pair<int, int>> goodNodes;

  std::queue<Node> partOfBestPath;

  for (auto end : ends)
  {
    auto d = distances[end];
    if (d == min)
    {
      partOfBestPath.push(end);
    }
  }

  while (!partOfBestPath.empty())
  {
    Node current = partOfBestPath.front();
    partOfBestPath.pop();

    goodNodes.insert(std::make_pair(std::get<1>(current), std::get<2>(current)));

    for (auto p : previous[current])
    {
      partOfBestPath.push(p);
    }
  }
